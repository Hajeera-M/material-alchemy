...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-background/90 backdrop-blur-md motion-header">
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-6 lg:px-12">
        <Link
          to="/"
          className="group flex items-baseline gap-2 transition-transform duration-500 hover:-translate-y-0.5"
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="text-mute transition-[color,transform] duration-500 hover:text-stone xl:hidden"
          >
      ...
      {open && (
        <nav aria-label="Mobile" className="mobile-menu border-t border-line xl:hidden">
          <div className="mx-auto max-w-[1440px] px-6 py-4">
            {navigation.map((item) => (
              <Link
                ...
                to={item.to}
                onClick={() => setOpen(false)}
                className="mobile-menu-item block border-b border-line py-3 font-display text-lg text-stone last:border-0"
              >
                {item.label}
              </Link>
            ))}
            <Link
              to="/contact"
              onClick={() => setOpen(false)}
              className="mobile-menu-item mt-4 inline-block border border-line px-4 py-2 label text-stone"
            >
              Start a project
            </Link>
          </div>
